...
def fill_na(X_df):
    X_df['gender'] = X_df['gender'].fillna(2)
    X_df['age_range'] = X_df['age_range'].fillna(0)
    return X_df

def compute_positive_rate(X, col, MIN_COUNT):
    ''' MIN_COUNT: we compute the rate for at least MIN_COUNT occurrences, 
    since if an item appears only once and it's a repeated buyer, we can hardly say the return rate is 100% '''
...
class FeatureExtractor():
    ...
    def transform(self, X_df):
        ...
        #dates_features = compute_dates(X_df, self.act_log)
